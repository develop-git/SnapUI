...
public extension SnapViewDSL {
    ...
    @discardableResult
    func safeMargin(_ edges: Rect, _ len: Float) -> Self {
    ...
    @discardableResult
    func margin(_ edges: Rect, _ len: Float) -> Self {
        let lv = CombinedLiteral(value: len)
        if edges.contains(.all) {
